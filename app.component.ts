...
export class AppComponent {
  ...
  deleteAPIData(d:any){
    console.log('deleteAPIData() called', d);
    for (var i = 0; i < this.selectedRowDetails.length; i++) {
      var obj = this.selectedRowDetails[i];
      if (obj.Col == d.Col) {
        this.selectedRowDetails.splice(i, 1)
      } 
    }
  }
  ...
  search() {
    ...
    if (this.selectedHealthPlanDetails == null || this.selectedHealthPlanDetails == undefined) {
      ...
      let d = JSON.stringify(this.selectedRowDetails);
      d = '"' + d  + '"';
      console.log("converted", d);
      this.selectedRowDetailsNew=[
        {
          "Col": "MemberID",
          "dtype": "str",
          "nullable": "N"
